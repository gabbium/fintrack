...
public class MovementSteps(TestFixture fx)
{
    ...
    public async Task<MovementDto> Given_ExistingMovement()
    {
        ...
        var response = await fx.Client.PostAsJsonAsync("/api/v1/movements", command);
    ...
    public async Task<HttpResponseMessage> When_AttemptToList(ListMovementsQuery query)
    {
        ...
        return await fx.Client.GetAsync("/api/v1/movements" + queryParams);
    }
    ...
    {
        return await fx.Client.PutAsJsonAsync("/api/v1/movements/" + id, command);
    }
