...
public class HealthTests(TestFixture fx) : TestBase
{
    ...
    {
        var response = await fx.UsersApiClient.GetAsync("/health/alive", TestContext.Current.CancellationToken);
        response.ShouldBeOk();
    }
    ...
    {
        var response = await fx.UsersApiClient.GetAsync("/health/ready", TestContext.Current.CancellationToken);
        response.ShouldBeOk();
    }
}
